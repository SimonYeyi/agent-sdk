...
class VolcStreamDecoderTest {
    ...
    fun `unknown event type yields empty list`() {
        val events = VolcStreamDecoder.decode("""{"type":"some.future.event","foo":"bar"}""")
    ...
    fun `session updated maps to Connected with session id`() {
        val events = VolcStreamDecoder.decode("""{"type":"session.updated","session":{"id":"s2"}}""")
        assertEquals(1, events.size)
    ...
    fun `session closed maps to Disconnected with session closed reason`() {
        val events = VolcStreamDecoder.decode("""{"type":"session.closed"}""")
        assertEquals(1, events.size)
    ...
    fun `input_audio_buffer committed is dropped`() {
        val events = VolcStreamDecoder.decode("""{"type":"input_audio_buffer.committed"}""")
        assertTrue(events.isEmpty())
    }

    @Test
    fun `transcription started maps to UserTranscriptStarted`() {
        val events = VolcStreamDecoder.decode(
    ...
    fun `response canceled maps to ResponseDone with empty id and CANCELED`() {
        val events = VolcStreamDecoder.decode("""{"type":"response.canceled"}""")
        assertEquals(1, events.size)
    ...
    fun `conversation item added is dropped`() {
        val events = VolcStreamDecoder.decode("""{"type":"conversation.item.added","items":[]}""")
        assertTrue(events.isEmpty())
    }
}