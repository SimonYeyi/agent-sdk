...
internal class RepairedMemory(private val underlying: Memory) : Memory by underlying {
    ...
    private suspend fun repairIfNot() {
        if (repaired) return
        repairOrphans(underlying, CRASHED)
        repaired = true
    }
    ...
    companion object {
         ...
         * - [CRASHED]：进程崩溃/被杀导致上次 run 无法现场收尾，下次 run 启动时修复。
         * - [CANCELLED]：本次 run 被协程取消，在 catch 块中当场修复。
         * - 调用方也可传入自定义文本。
         *
         * ## 不变量
         *
         ...
         */
        internal suspend fun repairOrphans(memory: Memory, reason: String) {
