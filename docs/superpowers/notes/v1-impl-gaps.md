...
| 维度 | 改动 |
|------|------|
| Agent 公共 API | `run` / `runStream` 两入口统一返回 `Flow<AgentEvent>`(共享 `loop` 内核);`AgentConfig.memory` 由 `AgentBuilder.memory(Memory)` 注入单实例,`run/runStream` 不再带 per-call memory 参数("一个 Agent = 一段对话") |
...
| **§5.1 ReAct 非流式算法** | Implemented (v1.1 重构) | `ReActAgent.run(input)` 返回 `Flow<AgentEvent>`；与 `runStream` 共享 `loop` 内核，差别仅在传入的 `llmCall = { req -> config.llmProvider.chat(req) }`(loop 内部统一从 `config.memory` 读写历史)。`ensureActive()`、`invokeTool` 吞业务异常不吞 `CancellationException`；业务异常触发 `onRunFailed` hook + emit `Failed` |
...
| **§8 错误处理** | Implemented (v1.1 收敛) | `AgentException` sealed class 含 `MaxIterations` / `LlmError` / `InvalidResponse` / `ToolNotFound` / `Unknown`;新增 `Throwable.toAgentException()` 扩展函数兜底工厂,边界处把任意 Throwable 抬升为 `AgentException`(已是 AgentException 则原样返回);`AgentEvent.Failed.cause` 与 `AgentHook.onRunFailed.cause` 收窄为 `AgentException`,Hook 不再接收 iteration 计数;Tool 业务异常转 `isError=true` 喂回 LLM;`finishReason=Length` 视为终态 |
...
**修复内容 (v1.1):**
- `ReActAgent` 提取共享 `loop` 内核,`run` / `runStream` 两条路径都通过 `loop` 执行
- `loop` 内部在 6 个时点统一调用 `invokeHooks`: `beforeLlmCall` / `afterLlmResponse` / `beforeToolCall` / `afterToolCall` / `onRunFailed` / `onRunCompleted`
