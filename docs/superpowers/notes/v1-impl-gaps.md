...
| **§8 错误处理** | Implemented (v1.1 收敛) | `AgentException` sealed class 含 `MaxIterations` / `LlmError` / `InvalidResponse` / `ToolNotFound` / `Unknown`;新增 `Throwable.toAgentException()` 扩展函数兜底工厂,边界处把任意 Throwable 抬升为 `AgentException`(已是 AgentException 则原样返回);`AgentEvent.Failed.cause` 与 `AgentHook.onError.cause` 收窄为 `AgentException`,Hook 不再接收 iteration 计数;Tool 业务异常转 `isError=true` 喂回 LLM;`finishReason=Length` 视为终态 |
