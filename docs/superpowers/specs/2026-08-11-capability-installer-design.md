# Capability Installer 抽象类设计

> 日期：2026-08-11 · 状态：**Proposed**
> 模块：`capability`（核心改动）+ `subagent` / `skill` / `toolset`（适配）
> 范围：在 capability 模块新增 `CapabilityInstaller` 抽象类作为模块实现者的接线契约；同时将 `CapabilityRegistry` / `CapabilityAdapter` / `DefaultCapabilityRegistry` 的泛型顺序统一调整为 `<C, T, Ctx>`。
...
- **单一契约类**：`CapabilityInstaller` 是**抽象类**，不是接口。所有接线逻辑（`installOn`）在抽象类里实现，模块作者只 override 部件点。
- **registry 是抽象方法**：`registry()` 与 `contextFactory()` / `arguments()` 并列作为抽象方法，override 与契约条目同处一处、可见性强，避免被混入构造器参数列表而被忽略。
- **行为参数化**：`enableDelegateAdaptMode` 是 `installOn(builder, ...)` 的参数，不是installer字段——是"装到哪个 builder"的临时决定，不是installer状态。
...
- **面向模块作者，不面向调用方**：抽象类是 `public`（契约公开），但各模块的 installer 类（`SubagentInstaller` / `SkillInstaller` / `ToolsetInstaller`）是 `internal`——调用方感知不到。

---

## 3. 核心类型

### 3.1 `capability` 模块 —— 新增 `CapabilityInstaller.kt`
...
2. `CapabilityInstaller` 抽象类的 `<C, T, Ctx>` 顺序与这三个对齐
3. 一致性收益大于一次性破坏成本

### 3.3 `subagent` 模块适配

新增 `SubagentInstaller.kt`：
```kotlin
...
- **`CapabilityInstallerTest.kt`**（位于 `agent/capability/src/test/...`）：用 `CapabilityRegistry` mock 测 `installOn` 委托、auxiliaryTools 遍历、参数透传
- **`SubagentInstallerTest.kt`**（位于 `agent/subagent/src/test/...`）：registry 持有、enableDelegateAdaptMode 透传、helper tool 安装
- **`SkillInstallerTest.kt`**（位于 `agent/skill/src/test/...`）：auxiliaryTools 条件返回
- **`ToolsetInstallerTest.kt`**（位于 `agent/toolset/src/test/...`）：`ToolsetsInstallException` wrap

测试粒度：覆盖"installer 正确把部件装到 AgentBuilder"集成点。`CapabilityAdapter` 行为细节由 `CapabilityAdapterTest` 覆盖，不重复。
...
1. 新增 `CapabilityInstaller.kt`（抽象类 + 默认 installOn）
...
4. 新增各模块的 `XxxInstaller.kt`
5. 重写各模块的 `XxxExtensions.kt` 为 ext fn → installer 委托
6. 同步更新 spec doc（`2026-06-23-subagent-design.md`）和 plan doc（`2026-07-15-team-module-impl.md`）中显式声明的泛型顺序
7. 新增 4 个 `XxxInstallerTest`，跑全量 `gradle test` 验证
...
| Toolset 的 try-catch 在抽象类 `installOn` 上 override，第三方扩展若有继承需要同步 | LOW | `installOn` 是 `open`，且 `ToolsetInstaller` 是 `internal`，第三方无法继承 |
...
- 异构 `List<CapabilityInstaller>` 列表支持（用户明确不需要）
