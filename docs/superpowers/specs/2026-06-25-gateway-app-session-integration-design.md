...
| 关联模块 | `gateway-app`(新,Android application,放在 `gateway/gateway-app/`,含 `MainActivity` + `GatewayService` + `DefaultAgentRunner` 三个类)、`:session`(`create` 加可选 `sessionId` 参数)、`gateway-core`(`AgentRunner` 接口不变,无文件改动) |
| 关联前置 | gateway 移植自 HermesApp(已合入主分支);`app` 模块已有的 `BuildConfig` + `local.properties` 注入模式 |
| 不在范围 | MainActivity UI 细节设计(只要有最小 launcher activity)、tools/skills/subagents 的 gateway-app 集成、gateway 平台适配器本身、多平台支持(本次只集成 feishu) |
...
└────────┬────────────────────────────────────────────────┘
         ...
         ▼
┌─────────────────────────────────────────────────────────┐
│  gateway-app (Android application)                       │
│                                                          │
│  ┌────────────────────────────────────────────┐          │
...
│  │          ▼                                │          │
│  │ ┌──────────────────────────────────────┐   │          │
│  │ │ DefaultAgentRunner (gateway-app)     │   │  ← 新增  │
...
- `gateway-app`(Android application,新)→ `gateway-core` + `:agent` + `:session` + `:providers:anthropic` + `:gateway:platforms:feishu` + Android SDK(`MainActivity` + `GatewayService`)
- `app`(Android application,现有)— **不依赖** `gateway-app`;通过 `Intent` 显式 `ComponentName` 触发 `gateway-app` 的 `GatewayService`(同 APK 内,显式启动,`exported="true"` 是为第三方 app 也能用)
...
### 4.2 gateway-app 整合 — `DefaultAgentRunner`(只接 lambda,不做工厂)

**改动文件**:
- `gateway/gateway-app/src/main/kotlin/io/github/yeyi/agent/gateway/app/DefaultAgentRunner.kt`(新建,包名 `io.github.yeyi.agent.gateway.app`)
...
**改动文件**:`gateway/gateway-app/src/main/kotlin/io/github/yeyi/agent/gateway/app/GatewayService.kt`(新建)
...
### 4.4 `gateway-app` 模块配置(Android application,可独立启动)

**新增文件**:
- `settings.gradle.kts`:`include(":gateway:gateway-app")`(与 `:gateway:gateway-core` / `:gateway:platforms:feishu` 同级,放在 gateway 子项目下)
- `gateway/gateway-app/build.gradle.kts`(新建)
- `gateway/gateway-app/src/main/AndroidManifest.xml`(新建)
- `gateway/gateway-app/src/main/kotlin/io/github/yeyi/agent/gateway/app/MainActivity.kt`(新建,launcher 入口;最小实现)
- `gateway/gateway-app/src/main/kotlin/io/github/yeyi/agent/gateway/app/GatewayService.kt`(见 4.3)
- `gateway/gateway-app/src/main/kotlin/io/github/yeyi/agent/gateway/app/DefaultAgentRunner.kt`(见 4.2)

**`gateway/gateway-app/build.gradle.kts`**(关键片段,`com.android.application` 插件,有 `applicationId` + `MainActivity`;实际文件顶部仍按惯例 `import java.util.Properties`,这里为简洁起见用 FQN):
```kotlin
plugins {
...
**`gateway/gateway-app/src/main/AndroidManifest.xml`**(同时声明 activity + service,各有 intent-filter):
```xml
...
**`local.properties`** 新增字段(已存在的 `MODEL_*` 字段保留,这里只列新增项;`:app` 模块目前不读这些字段,`:gateway:gateway-app` 读):
```properties
ANTHROPIC_API_KEY=...
...
**改动文件**:
- **同 APK 的 `:app` 模块**:在合适的入口(比如 `MainActivity.onCreate` / 通知栏 quick action / 应用启动器)用 `Intent` 显式 `ComponentName` 启动 `gateway-app` 的 Service;`:app` 模块**不**依赖 `:gateway:gateway-app`,只通过 action 字符串 + 类名耦合。
- **第三方 app**:用相同 action + 显式 `ComponentName`(或省略 component 走隐式 intent-filter)启动,需要本机安装 `gateway-app` APK。
- `:app/src/main/AndroidManifest.xml`:无需新增条目(不暴露 `gateway-app` 内部的 service,跨 APK 启动走 `gateway-app` manifest 里的 `<service>` exported="true")。
...
private fun startGateway(context: Context) {
    val intent = Intent(GatewayService.START).apply {
        setClassName(
            "io.github.yeyi.agent.gateway.app",                // gateway-app 的 applicationId
...
- **硬编码**:`:app` 里写一份 `const val START = "io.github.yeyi.agent.gateway.app.START"`(与 `gateway-app` 里的常量保持一致);坏处:两处都要维护,改一处忘改另一处就出 bug
- **共享常量模块**(推荐):抽一个 `:gateway:gateway-app:api` 子模块或 `:gateway:gateway-constants` 模块专门放 action / 类名字符串(只放 `const`,不放实现);`:app` 和 `:gateway:gateway-app` 都依赖它,避免重复维护
- 本次先**硬编码**:`:app` 模块与 `:gateway:gateway-app` 都在同一个代码仓库,改起来容易;后续如果 action / 类名被外部 app / 文档引用频繁,再抽共享模块

**为什么需要 `setClassName`**:第三方 app 走隐式 `Intent(action = ...)` 也能拉起(`<service>` 的 intent-filter 暴露了 action);但**显式 `ComponentName` 启动更稳** — 不依赖 intent-filter 匹配、不怕 service 被同名同 action 的其他 APK 抢去。`:app` 和 `gateway-app` 走同一 APK,显式启动是允许的(同进程同 UID)。

**为什么用 `startForegroundService`**:Android 8.0 (API 26) 起,后台启动 service 必须用 foreground service;`GatewayService.onStartCommand` 内部 `startForeground(...)` 必须配套调用(见 4.3)。`gateway-app` 的 `minSdk = 26`(见 4.4),所以统一走 `startForegroundService` 路径。
...
不在范围:消息重试 / 死信队列 / 会话过期清理 / Service 保活策略 — 这些是 gateway-app 装配时的策略选择,不在本次整合设计内。
...
| 文件 | 内容 |
|------|------|
| `gateway-app/src/test/kotlin/.../CreateAgentLambdaTest.kt` | 不存在 `AgentFactory` 类,改为**直接测试 lambda 体**:用临时目录 `File(tmp)` + `FakeLlmProvider` 构造 `SessionManager` + `LlmProvider`,写一个跟 `GatewayService` 里同形的 `createAgent` lambda,验证 lambda 首次调用走 `create` 分支建出新 session 且 `session.name == sessionName`、再次调用走 `get` 分支命中已存在 session;`session.memory` 在两次调用间持久化。**这套测试覆盖的就是 `GatewayService` 里那几行 lambda 体**(因为 lambda 是无依赖注入的纯逻辑,可以独立测) |
| `gateway-app/src/test/kotlin/.../DefaultAgentRunnerTest.kt` | 用 mock `createAgent` lambda(或记录调用的 fake lambda)验证:`process()` 把 `gatewaySession.platform` / `chatId` / `userId` 正确映射成 `accountId="gateway:<platform>"` / `sessionId="chatId:userId"`,并把 message text(或非文本回落 `sessionId`)作为 `sessionName` 转发给 `createAgent` |
| `gateway-app/src/test/kotlin/.../GatewayServiceAssemblyTest.kt`(可选) | 用 `Robolectric` 或自建 `Service` 替身(`Application` + `Context` mock)调 `onStartCommand`,验证:依赖图能完整拼装、`engine.start()` 被调用;不真起 engine / adapter,只验证装配路径 |
...
不写:
- 端到端飞书集成测试(需要真实平台凭证,在 `gateway-app` 文档里给出手动验证步骤即可 — 启动 app → service 自动起 → 飞书群里 @bot 发消息 → 验证回复 + 重启后 bot 仍记得上文)
...
| `gateway-app` 用 `com.android.application` + `Main.kt` 跑 JVM 长驻服务 | Android app 模块里塞 `application` 插件 + Ktor server 是一团混乱(manifest / `application` / 通知栏 / 前台 service 谁来拉 全要重排);`gateway-app` 走 `com.android.library` + `Service` 是 Android 原生做法,所有进程生命周期由系统托管 |
| `gateway-app` 用 `application` 插件 + Ktor server 收 webhook | feishu 已支持 native WebSocket 收消息,引入 Ktor 还得开端口 / 配反代 / 跑 HTTPS,徒增运维负担;Service + WebSocket 是 feishu 官方推荐路径 |
...
- `gateway-app` 接 MCP / Tools / Skills / Subagent(目前 app UI 那套 demo 不在 gateway-app 里)
...
- `gateway-app` 的 graceful shutdown(目前 `onDestroy` 直接 `engine?.stop()`,没等 in-flight message 跑完)
- 前台通知栏的快速操作按钮("停止 bot" / "查看状态" PendingIntent)
- `gateway-app` 模块用 instrumentation test 跑真 Service lifecycle(目前只覆盖装配路径,不真测 service 启动)
